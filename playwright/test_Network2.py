...
# video-52. Intercepting request calls with route->continue methods to test edge cases
...
# video-53. save and inject the session cookies into browser at run time with playwright
def test_session_storage(playwright: Playwright):
    ...
    page.add_init_script(f"""localStorage.setItem('token','{getToken}')""")  #script is inserted in javascript, so it is written within triple quotes. Any javascript initialization has to be wrapped in triple quotes
    page.goto("https://rahulshettyacademy.com/client")
    page.get_by_role("button", name="ORDERS").click()
