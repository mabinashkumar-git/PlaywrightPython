...
@pytest.fixture
def shared_data():
    return {}             # This fixture is returning empty dictionary
...
def user_on_landing_page(browserInstance, shared_data):        # browserInstance -> It's a fixture, it will go to conftest.py file and check
    loginPage = LoginPage(browserInstance)
    loginPage.navigate()
    shared_data['login_page'] = loginPage


@when(parsers.parse('I login to portal with {username} and {password}'))   # parsers.parse -> This reads {username}, curly braces as parameters
